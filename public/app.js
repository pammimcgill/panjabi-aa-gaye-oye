...
export async function mountEvents(){
  ...
  try{const data=await getJson('/api/events?limit=250');events=[...new Map((data.events||[]).map(e=>[e.title.toLowerCase().replace(/[^a-z0-9]/g,'')+'|'+e.startsAt.slice(0,10)+'|'+e.city.toLowerCase(),e])).values()];loading.classList.add('hidden');render()}catch(e){loading.classList.add('hidden');error.textContent=`Events could not load: ${e.message}`;error.classList.remove('hidden')}
}
